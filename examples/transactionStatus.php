...
$delim  = FALSE;
$delim_char = ',';
$result = $gateway->transactionStatus($txn_id, $delim, $delim_char);

if($result) {
  //The call succeeded, let's parse it out
  if($result['Result'] == '0'){
    //A "Result" of 0 typically means success
    echo "Transaction ID: " . $result['ACHTransaction_ID'] . " found.<br/>";
  } else {
    //Anything other than 0 specifies some kind of error.
    echo "Transaction not found.<br/>Error Code: {$result['Result']}<br/>Error: {$result['ResultDescription']}<br/>";
  }
